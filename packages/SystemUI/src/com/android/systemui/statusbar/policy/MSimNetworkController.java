...
public class MSimNetworkController extends NetworkController {
    ...
    private final void updateSimIcon(int cardIndex) {
        ...
        if (mMSimState[cardIndex] ==  IccCard.State.ABSENT) {
            ...
            if (DEBUG)
                Log.d(TAG, "updateSimIcon, no feature mNoMSimIconId[" + cardIndex
    ...
    protected void refreshViews(int subscription) {
        ...
        int N;
        if (DEBUG)
            Slog.d(TAG, "refreshViews[" + subscription + "] " + mMSimDataConnected[subscription]
                    + ", " + mMSimDataActivity[subscription] + ", " + mMSimState[subscription]);
