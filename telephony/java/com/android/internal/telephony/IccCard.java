...
public class IccCard {
    ...
    public IccCard(PhoneBase phone, String logTag, Boolean is3gpp, Boolean dbg) {
        Log.d(logTag, "[IccCard] Creating card type " + (is3gpp ? "3gpp" : "3gpp2"));
        mPhone = phone;
        this.is3gpp = is3gpp;
    ...
    public void dispose() {
        Log.d(mLogTag, "[IccCard] Disposing card type " + (is3gpp ? "3gpp" : "3gpp2"));
    ...
    public void registerForSimReady(Handler h, int what, Object obj) {
        ...
        synchronized (mStateMonitor) {
            ...
            if (getState() == State.READY && getSimState() == State.READY) {
                r.notifyRegistrant(new AsyncResult(null, null, null));
            }
        }
    }
    ...
    private void handleIccCardStatus(IccCardStatus newCardStatus) {
        ...
        State oldSimState, oldRuimState;

        oldState = mState;
        oldSimState = getSimState();
        oldRuimState = getRuimState();
        ...
        synchronized (mStateMonitor) {
            ...
            if (oldSimState != State.READY && getSimState() == State.READY) {
