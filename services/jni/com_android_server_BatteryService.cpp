...
static void android_server_BatteryService_update(JNIEnv* env, jobject obj)
{
    ...
    setBooleanField(env, obj, gPaths.unknownOnlinePath, gFieldIds.mUnknownOnline);
...
int register_android_server_BatteryService(JNIEnv* env)
{
    ...
    while ((entry = readdir(dir))) {
        ...
        if (length > 0) {
            ...
            if (strcmp(buf, "Mains") == 0) {
                ...
                snprintf(path, sizeof(path), "%s/%s/online", POWER_SUPPLY_PATH, name);
                if (access(path, R_OK) == 0)
                    gPaths.unknownOnlinePath = strdup(path);
            }
    ...
    if (!gPaths.unknownOnlinePath)
        LOGE("unknownOnlinePath not found");
    ...
    gFieldIds.mUnknownOnline = env->GetFieldID(clazz, "mUnknownOnline", "Z");
    ...
    LOG_FATAL_IF(gFieldIds.mUnknownOnline == NULL, "Unable to find BatteryService.UNKNOWN_ONLINE_PATH");
